...
public class WebSocketConnection implements WebSocket {
	...
	public void connect(URI webSocketURI,
	...
	public void connect(URI webSocketURI, String[] subprotocols,
			WebSocket.WebSocketConnectionObserver connectionObserver,
			WebSocketOptions options) throws WebSocketException {
		...
		if (webSocketURI == null) {
			...
			if (!mWebSocketURI.getScheme().equals(WS_URI_SCHEME)
					&& !mWebSocketURI.getScheme().equals(WSS_URI_SCHEME)) {
				throw new WebSocketException(
						"unsupported scheme for WebSockets URI");
			}

			this.mWebSocketSubprotocols = subprotocols;
			this.mWebSocketConnectionObserver = new WeakReference<WebSocket.WebSocketConnectionObserver>(
					connectionObserver);
	...
	private void connect() {
		...
		if (mSocket == null) {
			onClose(WebSocketCloseNotification.CANNOT_CONNECT,
					mSocketThread.getFailureMessage());
		} else if (mSocket.isConnected()) {
			try {
				createReader();
				createWriter();

				WebSocketMessage.ClientHandshake clientHandshake = new WebSocketMessage.ClientHandshake(
						mWebSocketURI, null, mWebSocketSubprotocols);
				mWebSocketWriter.forward(clientHandshake);
			} catch (Exception e) {
				onClose(WebSocketCloseNotification.INTERNAL_ERROR,
						e.getLocalizedMessage());
			}
		} else {
			onClose(WebSocketCloseNotification.CANNOT_CONNECT,
					"could not connect to WebSockets server");
		}
	}
	...
	protected boolean scheduleReconnect() {
		...
		boolean shouldReconnect = (mSocket == null && interval > 0)
				|| (mSocket.isConnected() && mPreviousConnection && (interval > 0));
		if (shouldReconnect) {
	...
	private void onClose(WebSocketCloseNotification code, String reason) {
		...
		if ((code == WebSocketCloseNotification.CANNOT_CONNECT)
				|| (code == WebSocketCloseNotification.CONNECTION_LOST)) {
		...
		if (webSocketObserver != null) {
			try {
				if (reconnecting) {
					webSocketObserver
							.onClose(
									WebSocketConnectionObserver.WebSocketCloseNotification.RECONNECT,
									reason);
				} else {
	...
	protected void createWriter() {
		mWebSocketWriter = new WebSocketWriter(mHandler, mSocket,
				mWebSocketOptions, WS_WRITER);
	...
	protected void createReader() {
		...
		mWebSocketReader = new WebSocketReader(mHandler, mSocket,
				mWebSocketOptions, WS_READER);
	...
	private void handleMessage(Message message) {
		...
		if (message.obj instanceof WebSocketMessage.TextMessage) {
			...
			Log.d(TAG, "WebSockets Close received (" + close.getCode() + " - "
					+ close.getReason() + ")");
			...
			// WebSocketMessage.ConnectionLost connectionLost =
			// (WebSocketMessage.ConnectionLost) message.obj;
			failConnection(WebSocketCloseNotification.CONNECTION_LOST,
			...
			// (WebSocketMessage.ProtocolViolation) message.obj;
			failConnection(WebSocketCloseNotification.PROTOCOL_ERROR,
			...
			failConnection(WebSocketCloseNotification.INTERNAL_ERROR,
			...
			failConnection(WebSocketCloseNotification.SERVER_ERROR,
	...
	public static class SocketThread extends Thread {
		...
		public void startConnection() {
			try {
				...
				// Do not replace host string with InetAddress or you lose
				// automatic host name verification
	...
	private static class ThreadHandler extends Handler {
		...
		public void handleMessage(Message message) {
			WebSocketConnection webSocketConnection = mWebSocketConnection
					.get();
