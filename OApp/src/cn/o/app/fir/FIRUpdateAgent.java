...
public class FIRUpdateAgent extends ContextOwnerTask {
	...
	public FIRUpdateAgent() {
		...
		mUpdateTask.addListener(new NetTaskListener<FIRUpdateTask.FIRUpdateReq, FIRUpdateTask.FIRUpdateRes>() {
			...
			public void onComplete(INetTask<FIRUpdateReq, FIRUpdateRes> task, FIRUpdateRes response) {
				...
				alert.setListener(new AlertListener() {
					...
					public boolean onOK(Alert alert) {
						if (mVersionUpdateListener != null) {
							mVersionUpdateListener.onUpdate(versionShort);
						}
						StringBuilder sb = new StringBuilder();
						sb.append(OUtil.getDiskCacheDir(mContext, "FIR"));
						sb.append(versionShort);
						sb.append(".apk");
						new HttpUtils().download(installUrl, sb.toString(), mDownloadCallBack);
						return false;
					}
					...
					public boolean onCancel(Alert alert) {
						if (mVersionUpdateListener != null) {
							mVersionUpdateListener.onUpdateCancel(versionShort);
						}
						return false;
					}
				});
