...
public class JsonUtil {
	...
	public static <T> T convertFromJson(Object json, Class<T> targetClass)
			throws Exception {
		return convertFromJson(json, targetClass, null);
	}

	protected static <T> T convertFromJson(Object json, Class<T> targetClass,
		...
		if (targetClass == String.class) {
			...
			return (T) ((json instanceof Integer) ? json : Integer.valueOf(json
					.toString()));
		} else if (targetClass == Long.TYPE || targetClass == Long.class) {
			return (T) ((json instanceof Long) ? json : Long.valueOf(json
					.toString()));
		} else if (targetClass == Double.TYPE || targetClass == Double.class) {
			return (T) ((json instanceof Double) ? json : Double.valueOf(json
					.toString()));
		} else if (targetClass == Boolean.TYPE || targetClass == Boolean.class) {
			return (T) ((json instanceof Boolean) ? json : Boolean.valueOf(json
					.toString()));
			...
			return (T) ReflectUtil.valueOfEnum(targetClass,
				...
				|| targetClass == JSONArray.class) {
			return (T) json;
		}
		return convertFromJson(json, targetClass.newInstance(), genericType);
	}

	protected static <T> T convertFromJson(Object json, T target,
			Type genericType) throws Exception {
		if (JSONObject.NULL.equals(json)) {
			return null;
		}
		...
		for (OField f : OField.getFields(target.getClass())) {
			...
			f.set(target,
					IJsonItem.class.isAssignableFrom(fClass) ? (((IJsonItem) fClass
							.newInstance()).fromJson(sub, f))
							: convertFromJson(sub, fClass, f.getGenericType()));
		}
		return target;
	}

	protected static <T extends List> T convertArrayFromJson(Object json,
	...
	protected static <T extends Map> T convertMapFromJson(Object json,
			T target, Type genericType) throws Exception {
	...
	public static <T> T convert(String str, Class<T> targetClass,
		...
		return convertFromJson(new JSONTokener(str).nextValue(), targetClass,
				genericType);
	}
	...
	public static <T> Object convertToJson(T target) throws Exception {
		...
		for (OField f : OField.getFields(target.getClass())) {
			...
			json.put(f.getName(),
					(v instanceof IJsonItem) ? (((IJsonItem) v).toJson(f))
							: convertToJson(v));
		}
		return json;
	}
