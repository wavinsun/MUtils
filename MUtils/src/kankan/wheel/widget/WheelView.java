...
public class WheelView extends View {
	...
	protected int mTextColorValue = VALUE_TEXT_COLOR;
	protected int mTextColorItems = ITEMS_TEXT_COLOR;
	protected boolean mFakeBoldText = true;
	...
	public void setFakeBoldText(boolean fakeBoldText) {
		...
		if (valuePaint != null) {
			valuePaint.setFakeBoldText(fakeBoldText);
			itemsPaint.setFakeBoldText(fakeBoldText);
			this.invalidate();
		}
	}
	...
	public void setTopShadowDrawable(GradientDrawable drawable) {
		topShadow = drawable;
		this.invalidate();
	}

	public void setBottomShadowDrawable(GradientDrawable drawable) {
		bottomShadow = drawable;
		this.invalidate();
	}
	...
	private void initResourcesIfNecessary() {
		if (itemsPaint == null) {
			...
			if (!mFakeBoldText) {
				itemsPaint.setFakeBoldText(mFakeBoldText);
			}
		}
		...
		if (valuePaint == null) {
			...
			if (!mFakeBoldText) {
				valuePaint.setFakeBoldText(mFakeBoldText);
			}
		}
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
			float textWidth = FloatMath.ceil(Layout.getDesiredWidth("国", itemsPaint));
	...
	private void createLayouts(int widthItems, int widthLabel) {
		...
		if (widthLabel > 0) {
			if (labelLayout == null || labelLayout.getWidth() > widthLabel) {
				labelLayout = new StaticLayout(label, valuePaint, widthLabel, Layout.Alignment.ALIGN_OPPOSITE, 1,
						ADDITIONAL_ITEM_HEIGHT, false);
			} else {
	...
	private void drawValue(Canvas canvas) {
		valuePaint.setColor(mTextColorValue);
		...
		if (labelLayout != null) {
			canvas.save();
			canvas.translate(itemsLayout.getWidth() + LABEL_OFFSET + PADDING / 2, bounds.top);
	...
	private void drawItems(Canvas canvas) {
		...
		itemsPaint.setColor(mTextColorItems);
